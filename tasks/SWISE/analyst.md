# Analyst: SWISE Token Framework Entry

Your job is to take the exhaustive research report and condense it into framework-compliant JSON entries: `swise-tokens.json` and `swise-metrics.json`.

## Step 1: Load the schema

Read the existing `tokens.json` and `metrics.json` from the forked ownership-token-framework repo (check the `swise` branch). Study:
...
Read `../output/swise-research.md` thoroughly. Map every finding to its corresponding framework criteria and metric.

## Step 3: Create the JSON entries

For `swise-tokens.json`:
- Token metadata and overall summary
- Category-level summaries
- Criteria-level summaries with scores

For `swise-metrics.json`:
...
Write the JSON files to `../output/` and commit to the `swise` branch.
