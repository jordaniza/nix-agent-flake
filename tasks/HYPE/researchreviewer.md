# Research Reviewer: HYPE Token Analysis
...
1. Read the research plan: `../output/hype-research-plan.md`
2. Read the research report: `../output/hype-research.md`
...
5. **On-chain verification**: For claims about current state (who holds a role, what address is set), call the view function yourself using `cast call` for EVM contracts or query the Hyperliquid API for L1 state. Does it match?

## Completeness check

For each metric, ask: **what else should be findable here?**

- If the report says "HYPE holders can vote on X" — where's the mechanism? What parameters? What quorum?
...
- If the report says "validators control X" — how are validators selected? Can HYPE stakers influence the validator set?

## Stress test

- Try to invalidate key claims. If the report says tokenholders control governance, look for a backdoor. An admin key. An upgradability path.
...
- Hyperliquid specific: is the validator set sufficiently decentralised? Can the team unilaterally change chain parameters?

## Do NOT approve if

- Any claim lacks a direct, verifiable source
- GitHub references don't include exact file paths and line numbers
- Contract addresses are not verified with Etherscan/explorer links
...
- Onchain state claims have not been verified with actual cast calls or API queries (log them)
...
4. [ ] Every onchain state claim verified with `cast call` or Hyperliquid API query. Calls and results logged.
...
8. [ ] Role matrix independently verified — every row confirmed with your own calls/queries.
