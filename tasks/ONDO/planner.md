# Planner: ONDO Token Analysis

Your job is to produce `../output/ondo-research-plan.md` — a structured research plan for analysing the ONDO token against the Aragon Ownership Token Framework.
...
1. State what you're looking for — what question does this metric answer for ONDO?
2. Create a longlist of Ondo Finance resources (contracts, repos, docs, governance forums, dashboards)
...
5. Flag any criteria where you anticipate difficulty or gaps — particularly around the RWA/regulatory boundary

## Step 5: Save to GitHub

Commit the research plan to `research/ondo-research-plan.md` on the `ondo` branch. Also write it to `../output/ondo-research-plan.md` for the pipeline.

## What the plan must contain

- **Resource inventory**: every Ondo Finance source you've identified (repos, contracts, docs, forums, dashboards) with URLs. Confirm each URL is accessible.
...
- **Gaps and concerns**: a section noting potential gaps, areas of difficulty, or criteria where evidence may be weak or absent. Note specifically where the RWA/regulatory context may limit what can be verified onchain.
