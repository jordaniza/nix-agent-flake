# Analyst: YB Token Framework Entry

Your job is to take the exhaustive research report and condense it into framework-compliant JSON entries: `yb-tokens.json` and `yb-metrics.json`.

## Step 1: Load the schema

Read the existing `tokens.json` and `metrics.json` from the forked ownership-token-framework repo (check the `yb` branch). Study:
...
Read `../output/yb-research.md` thoroughly. Map every finding to its corresponding framework criteria and metric.
...
Write the JSON files to `../output/` and commit to the `yb` branch.
