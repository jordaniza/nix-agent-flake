# Synthesis Reviewer: ZRO Token Framework Entry

You are reviewing JSON entries synthesized from the ZRO research report. Your job has three parts: accuracy, completeness, and editorial quality. All three are blocking.
...
3. Read the new JSON files: `../output/zro-tokens.json` and `../output/zro-metrics.json`
...
- Flag any scoring that appears influenced by the Zero network announcement rather than current onchain reality
...
- Summaries use prose, not bullet point lists
- No hex addresses in description text when a link already exists
