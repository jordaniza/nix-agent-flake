# Plan Reviewer: ZRO Token Analysis

You are reviewing a research plan for the ZRO token against the Aragon Ownership Token Framework.
...
3. Read the research plan in `../output/zro-research-plan.md`
...
### Token focus
- Is the plan focused on what the ZRO **token** gives its holder, not on LayerZero as a protocol/company?
...
- Does the plan separate current ZRO utility from the speculative future utility on the Zero network?
...
- Flag any criteria where the evidence would be largely unfalsifiable (e.g. "the team intends to decentralize" or "Zero network will use ZRO")
...
- The plan scores based on the Zero network announcement rather than current onchain reality
