# Task: Apply Aragon Ownership Token Framework to ENA (Ethena)
...
1. **What do I own?** — When I buy the ENA token, what do I have unilateral, unalienable control over as a tokenholder?
2. **Why should it have value?** — What gives the ENA token economic value?
...
### 1. Research Plan (`output/ena-research-plan.md`)

A structured plan mapping each framework criteria and metric to specific Ethena resources and investigation approach. This is the blueprint for the research phase.

### 2. Research Report (`output/ena-research.md`)

An exhaustive, fully-sourced research document covering ENA across every framework category. Every claim must have a direct, verifiable source — GitHub line numbers, contract addresses, view function outputs, or documentation links.

### 3. Framework Data (`output/ena-tokens.json`, `output/ena-metrics.json`)

JSON entries for ENA matching the exact schema of existing entries in `tokens.json` and `metrics.json`. Must be traceable to the research report.

### 4. Deployed Site

The branch with new ENA data merged and deployed to Vercel. Build must succeed, site must render correctly with the new token.
...
2. Create an `ena` branch from `development`
...
This analysis is about the **ENA token**, not the Ethena protocol or company. The question is not "is Ethena a good protocol?" but "does the ENA token give its holder meaningful, enforceable control and economic value?"
