# Analyst: ENA Token Framework Entry

Your job is to take the exhaustive research report and condense it into framework-compliant JSON entries: `ena-tokens.json` and `ena-metrics.json`.

## Step 1: Load the schema

Read the existing `tokens.json` and `metrics.json` from the forked ownership-token-framework repo (check the `ena` branch). Study:
...
Read `../output/ena-research.md` thoroughly. Map every finding to its corresponding framework criteria and metric.
...
Write the JSON files to `../output/` and commit to the `ena` branch.
