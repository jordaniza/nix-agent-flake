# Synthesis Reviewer: ENA Token Framework Entry

You are reviewing JSON entries synthesized from the ENA research report. Your job has three parts: accuracy, completeness, and editorial quality. All three are blocking.

## Load context

1. Read existing entries in `tokens.json` and `metrics.json` from the ownership-token-framework repo for schema reference
2. Read the research report: `../output/ena-research.md`
3. Read the new JSON files: `../output/ena-tokens.json` and `../output/ena-metrics.json`
